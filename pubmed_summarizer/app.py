...
<div class="main-card">
    ...
    <div class="main-sub">
        Machine Learning-Based Extractive NLP System using TF-IDF and Logistic Regression
    </div>
</div>
""", unsafe_allow_html=True)
...
def load_model():
    if not os.path.exists(MODEL_PATH):
        raise Exception("Run train_model.py first")
...
with tab3:
    ...
    df = pd.DataFrame({"Sentence":sents,"Score":scores}).sort_values("Score", ascending=False)
...
with tab4:
    st.markdown("<div class='block'>", unsafe_allow_html=True)
    st.write("""
TF-IDF + Logistic Regression is used for sentence scoring.
Top sentences are selected to generate extractive summaries of biomedical text.
""")
    st.markdown("</div>", unsafe_allow_html=True)
